...
#ifndef SWEEP_CHUNK
#define SWEEP_CHUNK 4096
#endif
#ifndef SWEEP_THREADS
#define SWEEP_THREADS 256
#endif

namespace psearch {

static constexpr int kChunk    = SWEEP_CHUNK;
static constexpr int kThreads3 = SWEEP_THREADS;
...
__global__ void build_lut(
    const float* __restrict__ queries, int n_q,
...
__global__ void knn_v3_pq_fat(
    const uint8_t* __restrict__ codes, int n_db,
    const float* __restrict__ lut, int n_q,
    ...
    for (int i = tid; i < kPQSub * kPQCent; i += kThreads3) s_lut[i] = lut_q[i];
    __syncthreads();

    const int c_start = chunk_id * kChunk;
    const int c_end   = min(c_start + kChunk, n_db);

    float my_d[kK]; int my_i[kK];
    #pragma unroll
    ...
    for (int c = c_start + tid; c < c_end; c += kThreads3) {
        ...
        float dist = 0.0f;
        #pragma unroll
        for (int m = 0; m < kPQSub; ++m) dist += s_lut[m * kPQCent + code[m]];
